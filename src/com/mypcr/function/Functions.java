	...
	public static void logTemperature( double chamber, double lid, double heatsink, boolean isFirst )
	{
		if( isLogging )
		{
			...
			if( isFirst )
			{
				...
				tempLogFilePath = tempLogPath
						+ ( isMac ? ( "/" + serialNumber + "tempLog.txt" ) : ( "\\" + serialNumber + "tempLog.txt" ) );
			...
			String data = String.format( "%8d\t%d\t%8.1f\t%8.1f\t%8.1f", tempLogCounter, endTime - tempLogStartTime,
					chamber, lid, heatsink );
